...
class VARGLLVM(nn.Module):
    ...
    def __init__(self, num_var, num_latent, num_covar, response_types, response_args=None, response_link=None, response_linkinv=None, VAR1_intercept=False, VAR1_slope=False, fixed_first_loading=False, linpred_max = 20, logvar_z1=None):
        super().__init__()
        self.settings = VARGLLVMSettings(
            p=num_var,
            q=num_latent,
            k=num_covar,
            family=response_types,
            args=response_args,
            link=response_link,
        ...
        # Define Modules
        # --------------
        self.VAR1 = VAR1(A=self.A, logvar_z1 = self.logvar_z1, beta_0 = , beta_1 = self.VAR1_slope)
    ...
    def sample_response(self, mean):
        ...
        for response_type, response_id in self.response_types.items():
            if response_type == "binomial":
                y[:,:,response_id] = torch.binomial(mean[:,:,response_id]).to(device)
